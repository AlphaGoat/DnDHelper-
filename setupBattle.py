...
parser.add_argument('-p', '--players', type=str, action='append',
                help="Names of human players in battle")
...
class BattleScenario(object):
	...
	def __init__(self, creatureDict, playerDict):
    ...
    def determine_battle_challenge_rating(self):
    ...
    def npc_action(self, npc_character):
        ...
        while npc_turn:
            ...
            print('\n',bMessager.npc_turn_initiate(npc_character.name))
            print('\n----------Available Attacks---------------\n')
            attacks_list = list()
            attacks_dict = npc_character.actions['attacks']
            ...
            print("\nSelect a target: ")
            next_target = bH.select_a_target(npc_character,
	...
	def initBattleSequence(self):
		...
		for creat in self.orderedCreatureList:
			if creat.allegiance == 'friend':
				++ self.friends
			else if creat.allegiance == 'foe':
				++ self.foes
			else if creat.allegiance == 'neutral':
				++ self.swiss
...
#						return
	...
	def playerAction(self, player):
		...
		while playerTurn:
            ...
            print("\nSkip turn?")
            response = input(" (y/n): \n")
            if response == 'y' or response == 'Y': playerTurn=False
    ...
    def check_for_allegiance_change(self, character):
        print("\nChange allegiance for {}?".format(character))
        confirmation = input("\n(y/N): ")
            ...
            if confirmation.lower() == 'n' or confirmation.lower() == 'no':
                ...
                while True:
                    newAllegiance = input('''\nSelect what allegiance 
                                to set {} to (fr/fo/ne)'''.format(character))
    ...
    def script(self):
        ...
        while initBattle:
            ...
            nextCharacter = self.orderedCreatureList[idx]
            ...
            # accordingly
            if nextCharacter.npc == 1:
                # Check if the DM wants to change the NPC's allegiance
                self.check_for_allegiance_change(nextCharacter)
                # Let npc take action
                self.npcAction(nextCharacter)

            else:
                self.playerAction(nextCharacter)
...
if __name__ == '__main__':
    ...
    npc_creatures = bH.parse_battle_csv(csv_file)
    ...
    for npc_creature in npc_creatures:
        ...
        creat_type = npc_creature['type']
        allegiance = npc_creature['allegiance']
        creatureDict[npc_creature['name']] = mYP.Creature(name, 
                                    creat_type, allegiance=allegiance,
                                    )
